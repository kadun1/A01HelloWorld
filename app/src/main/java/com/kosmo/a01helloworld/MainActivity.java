...
public class MainActivity extends AppCompatActivity {
    ...
    //전화번호 입셜상자를 전역적으로 사용하기 위해 선언
    private EditText editText;
    /*
    Java에서 출발점은 main()메소드였듯이, 안드로이드의 출발점은
    onCreate()메소드이다. 수명주기(Lifecycle) 메소드 중에서
    첫번째로 실행된다.
     */
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        ...
        btnNate.setOnClickListener(new View.OnClickListener(){
            ...
            public void onClick(View v) {
                /*
                버튼을 눌렀을때 토스트 메세지를 띄워준다. 토스트는 JS의
                alert()와 비슷하게 메세지를 일정시간 보여준다.
                형식] Toast.makeText(메세지를 띄울 컨텍스트(뷰),
                                    메세지내용,
                                    시간(Long or Short)).show();
                 */
                Toast.makeText(v.getContext(), "잠시후 네이트로 이동합니다",
                        Toast.LENGTH_LONG).show();
                /*
                인텐트를 통해 새로운 액티비티를 띄워준다. 웹 URL이
                전달되므로 외장 웹브라우저를 통해 네이트를 접속하게된다.
                 */
    ...
    };//View.OnClickListener 끝

    /*
    리스너 부착방법4 : 버튼의 onClick속성을 이용한다. 이때 호출할
        메소드명을 기술하면 아래 함수를 즉시 호출할수 있다.
     */
    public void myBtnClick(View view){
        Toast.makeText(view.getContext(),
                "onClick속성을 이용한 버튼입니다. 저는 짧아요ㅋ",
